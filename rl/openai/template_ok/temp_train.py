...
from rl.openai.template_ok.temp_env import TempEnv, WIDTH, HEIGHT
...
class DQN:
    ...
    def replay(self):
        ...
        for sample in samples:
            ...
            losses.append(loss)
        logging.info("loss avg is {}".format(np.mean(losses)))
...
def train(display=False):
    ...
    for trial in range(trials):
        ...
        for step in range(trial_len):
            ...
            if done:
                break
        logging.info("step {} complete with e {:.4f}. mem {}".format(step, dqn_agent.epsilon, len(dqn_agent.memory)))
